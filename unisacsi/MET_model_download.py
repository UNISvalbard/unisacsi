...
"""
    ...
    def download_nearsurface_timeseries(self):
        ...
        for i, (stat, lon, lat) in enumerate(zip(self.stt_name, self.stt_lon, self.stt_lat)):
            coords_xx[i], coords_yy[i], model_lon[i], model_lat[i] = Mmt.lonlat2xy(lon, lat, self.static_fields['lon'], self.static_fields['lat'], 1)
        ...
        for qr, stat in enumerate(self.stt_name):           # loop over stations
            ...
            if "ur" in varis_to_load:
                ...
                data[stat]['u'], data[stat]['v'] = Mmt.Rotate_uv_components(data[stat]['ur'], data[stat]['vr'],coords_xx[qr], coords_yy[qr], self.static_fields["lon"],1,self.model)
    ...
    def download_nearsurface_fields(self):
        ...
        start_lonlat, count_lonlat, _, _ = Mmt.lonlat2xy(self.lon_lims, self.lat_lims, self.static_fields["lon"], self.static_fields["lat"], 2)
        ...
        if "ur" in varis_to_load:
            ...
            for nn in range(len(time)):
                data['u'][:,:,nn], data['v'][:,:,nn] = Mmt.Rotate_uv_components(data['ur'][:,:,nn], data['vr'][:,:,nn],idxx, idyy, self.static_fields["lon"],2,self.model)
    ...
    def download_cross_section(self):
        ...
        if "ur" in varis_to_load:
            ...
            for ml in range(self.model_levels):
                for nn in range(len(time)):
                    data['u'][:,:,ml,nn], data['v'][:,:,ml,nn] = Mmt.Rotate_uv_components(data['ur'][:,:,ml,nn], data['vr'][:,:,ml,nn],idxx, idyy, self.static_fields["lon"],2,self.model)
