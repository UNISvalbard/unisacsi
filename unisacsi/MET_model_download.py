...
def download_MET_model_data(config_file):
    ...
    if config_settings["latest"]:
        config_settings["out_path"] = (
            f"{config_settings['out_path']}_{config_settings['resolution']}.nc"
        )
        print("############################################################")
        ...
        days = pd.date_range(
            config_settings["start_day"], config_settings["end_day"], freq="1D"
        )
        ...
        for d in days:
            ...
            daily_config["out_path"] = (
                f"{'/'.join(path)}/{filename}_{config_settings['resolution']}_{d.strftime('%Y%m%d')}.nc"
            )
            print("############################################################")
    ...
    if model == "AA":
        if resolution == "2p5km":
            ...
            with xr.open_dataset(file) as static_fields:
                ...
                ].squeeze().to_netcdf(out_path, unlimited_dims=())

        elif resolution == "500m":
            file_1 = (
                "https://thredds.met.no/thredds/dodsC/finis/UNIS-2024/AA_2024012400.nc"
            )
...
class MET_model_download_class:
    ...
    def __init__(self, config_settings):
        ...
        self.lon_lims = [
            config_settings["area"]["lon_min"],
            config_settings["area"]["lon_max"],
        ]
        self.lat_lims = [
            config_settings["area"]["lat_min"],
            config_settings["area"]["lat_max"],
        ]
        self.int_x = config_settings["int_x"]
        self.int_y = config_settings["int_y"]
        self.resolution = config_settings["resolution"]
        self.start_point = [
            config_settings["section"]["start_lat"],
            config_settings["section"]["start_lon"],
        ]
        self.end_point = [
            config_settings["section"]["end_lat"],
            config_settings["section"]["end_lon"],
        ]
        ...
        if self.model == "AA":
            if self.resolution == "2p5km":
                ...
                self.time_vec = pd.date_range(
                    self.start_time, self.end_time, freq="1D", inclusive="left"
                )
            else:
                assert False, "Resolution not valid, specify either '2p5km' or '500m'."
        elif self.model == "MC":
            self.time_vec = pd.date_range(
                self.start_time, self.end_time, freq=f"{self.int_f}h", inclusive="left"
            )
        else:
        ...
        if self.model == "AA":
            ...
            if self.resolution == "2p5km":
                if self.latest:
                    self.fileurls.append(
                        "https://thredds.met.no/thredds/dodsC/aromearcticlatest/archive/arome_arctic_det_2_5km_latest.nc"
                    )
                else:
                        ...
                        if t < pd.Timestamp("2022-02-01"):
                            if config_settings["data_format"] == 6:
                                ...
                                if self.shortest_leadtime:
                                    if file in threddsclient.opendap_urls(
                                        f'https://thredds.met.no/thredds/catalog/aromearcticarchive/{t.strftime("%Y/%m/%d")}/catalog.html'
                                    ):
                                        self.fileurls.append(file)
                                        self.time_ind.append(
                                            np.arange(
                                                self.start_h,
                                                self.start_h + self.num_h,
                                                self.int_h,
                                                dtype=int,
                                            )
                                        )
                ...
                for t in self.time_vec:
                    if self.aa500_folder == "UNIS-2024":
                        self.fileurls.append(
                            f'https://thredds.met.no/thredds/dodsC/finis/{self.aa500_folder}/{aa500_filename[self.aa500_folder]}_{t.strftime("%Y%m%d")}{aa500_hour[self.aa500_folder]}.nc'
                        )
                    else:
                        self.fileurls.append(
                            f'https://thredds.met.no/thredds/dodsC/metusers/yuriib/{self.aa500_folder}/{aa500_filename[self.aa500_folder]}_{t.strftime("%Y%m%d")}{aa500_hour[self.aa500_folder]}.nc'
                        )
                    self.time_ind.append(
                        np.arange(
                            self.start_h,
            ...
            for t in self.time_vec:
                ...
                if self.shortest_leadtime:
                    if file in threddsclient.opendap_urls(
                        f'https://thredds.met.no/thredds/catalog/meps25epsarchive/{t.strftime("%Y/%m/%d")}/catalog.html'
                    ):
                        self.fileurls.append(file)
                        self.time_ind.append(
                            np.arange(
                        ...
                        self.time_ind.append(
                            np.arange(
                                l[0], l[-1] + 1 + self.num_h, self.int_h, dtype=int
                            )
                        )
                else:
                    self.fileurls.append(file)
        ...
        if "data_format" in config_settings.keys():
            if config_settings["data_format"] == 0:
                ...
                assert (
                    False
                ), "Data format not a valid option, please change in the config file."
        else:
    ...
    def download_profile_data(self):
        ...
        for v in self.varis:
            try:
                ...
                print(
                    f"The variable {v} is unfortunately not available as {self.data_format}, please de-select in the configuration file and try again."
                )
                sys.exit(1)
        ...
        model_varis = list(varis_to_load.values()) + [
            "x",
            "y",
            "longitude",
            "latitude",
        ...
        for i in range(len(self.stt_lon)):
            x, y = data_crs.transform_point(
                self.stt_lon[i], self.stt_lat[i], src_crs=ccrs.PlateCarree()
            )
            xx.append(x)
            yy.append(y)
        ...
        if self.check_plot:
            ...
            lon_min = (
                np.nanmin(
                    [
                        self.static_fields.sel(x=xx[i], y=yy[i], method="nearest")[
                            "longitude"
                        ]
                        for i in range(len(self.stt_name))
                    ]
                )
                - 1.0
            )
            ...
            for i, (lon, lat) in enumerate(zip(self.stt_lon, self.stt_lat)):
                ax.scatter(
                    lon, lat, s=10.0, c="r", marker="x", transform=ccrs.PlateCarree()
                )
                ax.scatter(
                    self.static_fields.sel(x=xx[i], y=yy[i], method="nearest")[
                        "longitude"
            ...
            print(
                "Type 'Y' to proceed to the download. Any other input will terminate the script immediately."
            )
            print("")
        ...
        chunks = []
        for filename, time_ind, hyb_name in zip(
            self.fileurls, self.time_ind, self.hybrid_name
        ):
            stations = []
                ...
                for i in range(len(self.stt_lon)):
                    if hyb_name == "":
                        stations.append(
                            full_file.isel(
                                time=time_ind,
                                hybrid=np.arange(-self.model_levels, 0, 1),
                            )
                            .sel(x=xx[i], y=yy[i], method="nearest")[model_varis]
                            .squeeze()
                        )
                    else:
                        ds_temp = (
                            full_file.isel(
                                time=time_ind,
                                hybrid0=np.arange(-self.model_levels, 0, 1),
                            )
                            .sel(x=xx[i], y=yy[i], method="nearest")[model_varis]
                            .squeeze()
                        )
            ...
            data = xr.concat(stations, dim="station", data_vars="all")

            chunks.append(data)

            print(f"Done downloading {filename}.")

        ds = xr.concat(chunks, dim="time", data_vars="all")
        ...
        if "x_wind_ml" in model_varis:
            ...
            cone = np.sin(
                np.abs(
                    np.deg2rad(
                        ds.projection_lambert.attrs["latitude_of_projection_origin"]
                    )
                )
            )  # cone factor

            ds["diffn"] = (
                ds.projection_lambert.attrs["longitude_of_central_meridian"]
                - ds.longitude
            )
            ...
            ds["eastward_wind_ml"] = ds["x_wind_ml"] * np.cos(ds["alpha"]) - ds[
                "y_wind_ml"
            ] * np.sin(ds["alpha"])
            ds["northward_wind_ml"] = ds["y_wind_ml"] * np.cos(ds["alpha"]) + ds[
                "x_wind_ml"
            ] * np.sin(ds["alpha"])

            # Calculating wind direction
            ds["wind_direction_ml"] = (
                np.rad2deg(
                    np.arctan2(-ds["eastward_wind_ml"], -ds["northward_wind_ml"])
                )
                + 360.0
            ) % 360.0

            # Calculating wind speed
            ds["wind_speed_ml"] = np.sqrt(
                (ds["eastward_wind_ml"] ** 2.0) + (ds["northward_wind_ml"] ** 2.0)
            )
        ...
        ds["air_potential_temperature_ml"] = (ds["air_temperature_ml"]) * (
            (1.0e5 / (ds["air_pressure_ml"])) ** (287.0 / 1005.0)
        )

        if "specific_humidity_ml" in model_varis:
            T = ds["air_temperature_ml"] - 273.15
            e = (ds["specific_humidity_ml"] * ds["air_pressure_ml"]) / (
                0.622 + 0.378 * ds["specific_humidity_ml"]
            )
    ...
    def download_near_surface_time_series_data(self):
        ...
        for filename, time_ind in zip(self.fileurls, self.time_ind):
            ...
            for vari in model_varis:
                if vari[:8] == "integral":
                    data[vari][dict(time=range(1, len(data["time"])))] -= data[vari][
                        dict(time=range(0, len(data["time"]) - 1))
                    ].values
                    data[vari][dict(time=0)] /= self.start_h
                    data[vari] /= 3600.0 * self.int_h
                    data[vari].attrs["standard_name"] = data[vari].attrs[
                        "standard_name"
                    ][12:-9]
        ...
        if "x_wind_10m" in model_varis:
            ...
            ds["wind_direction_10m"] = (
                np.rad2deg(
                    np.arctan2(-ds["eastward_wind_10m"], -ds["northward_wind_10m"])
                )
                + 360.0
            ) % 360.0
        ...
        # Calculating potential temperature
        if ("air_temperature_2m" in model_varis) & (
            "surface_air_pressure" in model_varis
        ):
    ...
    def download_near_surface_field_data(self):
        ...
        yy = []
        x, y = data_crs.transform_point(
            self.lon_lims[0], self.lat_lims[0], src_crs=ccrs.PlateCarree()
        )
        xx.append(self.static_fields["x"].sel(x=x, method="nearest"))
        yy.append(self.static_fields["y"].sel(y=y, method="nearest"))
        x, y = data_crs.transform_point(
            self.lon_lims[1], self.lat_lims[0], src_crs=ccrs.PlateCarree()
        )
        xx.append(self.static_fields["x"].sel(x=x, method="nearest"))
        yy.append(self.static_fields["y"].sel(y=y, method="nearest"))
        x, y = data_crs.transform_point(
            self.lon_lims[1], self.lat_lims[1], src_crs=ccrs.PlateCarree()
        )
        xx.append(self.static_fields["x"].sel(x=x, method="nearest"))
        yy.append(self.static_fields["y"].sel(y=y, method="nearest"))
        x, y = data_crs.transform_point(
            self.lon_lims[0], self.lat_lims[1], src_crs=ccrs.PlateCarree()
        )
        xx.append(self.static_fields["x"].sel(x=x, method="nearest"))
        yy.append(self.static_fields["y"].sel(y=y, method="nearest"))
        ...
        if self.check_plot:
            ...
            ax.set_title(None)
            pic = ax.pcolormesh(
                self.static_fields["longitude"].sel(x=x, y=y),
                self.static_fields["latitude"].sel(x=x, y=y),
                (self.static_fields["surface_geopotential"].sel(x=x, y=y)) / 9.81,
                cmap=mpl.cm.terrain,
                shading="nearest",
                transform=ccrs.PlateCarree(),
            )
        ...
        for filename, time_ind in zip(self.fileurls, self.time_ind):
            with xr.open_dataset(filename) as full_file:
                data = (
                    full_file.isel(time=time_ind).sel(x=x, y=y)[model_varis].squeeze()
                )
        ...
        for filename, time_ind, hyb_name in zip(
            ...
            with xr.open_dataset(filename) as full_file:
                if hyb_name == "":
                    data = (
                        full_file.isel(
                            time=time_ind, hybrid=np.arange(-self.model_levels, 0, 1)
                        )
                        .sel(x=x, y=y)[model_varis]
                        .squeeze()
                    )
                else:
                    data = (
                        full_file.isel(
                            time=time_ind, hybrid0=np.arange(-self.model_levels, 0, 1)
                        )
                        .sel(x=x, y=y)[model_varis]
                        .squeeze()
                    )
    ...
    def download_vertical_cross_section_data(self):
        ...
        yy = []
        x, y = data_crs.transform_point(
            self.start_point[1], self.start_point[0], src_crs=ccrs.PlateCarree()
        )
        xx.append(self.static_fields["x"].sel(x=x, method="nearest"))
        yy.append(self.static_fields["y"].sel(y=y, method="nearest"))
        x, y = data_crs.transform_point(
            self.end_point[1], self.end_point[0], src_crs=ccrs.PlateCarree()
        )
        xx.append(self.static_fields["x"].sel(x=x, method="nearest"))
        yy.append(self.static_fields["y"].sel(y=y, method="nearest"))

        x = np.arange(
            np.nanmin(xx) - 3 * self.dxdy,
            (np.nanmax(xx) + 3 * self.dxdy + 1),
            self.dxdy,
        )
        y = np.arange(
            np.nanmin(yy) - 3 * self.dxdy,
            (np.nanmax(yy) + 3 * self.dxdy + 1),
            self.dxdy,
        )
        x = x[:: self.int_x]
        y = y[:: self.int_y]

        xp, yp = data_crs.transform_point(
            self.end_point[1], self.start_point[0], src_crs=ccrs.PlateCarree()
        )
        xx.append(self.static_fields["x"].sel(x=xp, method="nearest"))
        yy.append(self.static_fields["y"].sel(y=yp, method="nearest"))
        xp, yp = data_crs.transform_point(
            self.start_point[1], self.end_point[0], src_crs=ccrs.PlateCarree()
        )
        xx.append(self.static_fields["x"].sel(x=xp, method="nearest"))
        yy.append(self.static_fields["y"].sel(y=yp, method="nearest"))
        ...
        if self.check_plot:
            ...
            ax.set_extent(
                [
                    xp[0] - 0.1 * abs(xp[-1] - xp[0]),
                    xp[-1] + 0.1 * abs(xp[-1] - xp[0]),
                    yp[0] - 0.1 * abs(yp[-1] - yp[0]),
                    yp[-1] + 0.1 * abs(yp[-1] - yp[0]),
                ],
                crs=data_crs,
            )
            ax.coastlines(resolution="10m", linewidth=0.5, zorder=30)
            ...
            ax.set_title(None)
            pic = ax.pcolormesh(
                self.static_fields["longitude"].sel(x=xp, y=yp, method="nearest"),
                self.static_fields["latitude"].sel(x=xp, y=yp, method="nearest"),
                (
                    self.static_fields["surface_geopotential"].sel(
                        x=xp, y=yp, method="nearest"
                    )
                )
                / 9.81,
                cmap=mpl.cm.terrain,
                ...
                if ("x" in ds[vari].dims) and ("y" in ds[vari].dims):
                    ...
                    cross_sections.append(
                        metpy.interpolate.cross_section(
                            ds[vari], self.start_point, self.end_point
                        )
                    )
        ...
        cross_section["distance"] = 1.0e-3 * np.sqrt(
            (cross_section.x - cross_section.x.values[0]) ** 2.0
            + (cross_section.y - cross_section.y.values[0]) ** 2.0
        )
    ...
    def download_horizontal_near_surface_section_data(self):
        ...
        for filename, time_ind in zip(self.fileurls, self.time_ind):
            ...
            for vari in model_varis:
                if vari[:8] == "integral":
                    data[vari][dict(time=range(1, len(data["time"])))] -= (
                        data[vari].diff(dim="time").values
                    )
                    data[vari][dict(time=0)] /= self.start_h
                    data[vari] /= 3600.0 * self.int_h
                    data = data.rename({vari: vari[12:-9]})
    ...
    def download_pressure_level_data(self):
        ...
        for filename, time_ind in zip(self.fileurls, self.time_ind):
            with xr.open_dataset(filename) as full_file:
                data = (
                    full_file.isel(time=time_ind, hybrid=ind_p_levels)
                    .sel(x=x, y=y)[model_varis]
                    .squeeze()
                )
        ...
        if "x_wind_pl" in model_varis:
            ...
            ds["eastward_wind_pl"] = ds["x_wind_pl"] * np.cos(ds["alpha"]) - ds[
                "y_wind_pl"
            ] * np.sin(ds["alpha"])
            ds["northward_wind_pl"] = ds["y_wind_pl"] * np.cos(ds["alpha"]) + ds[
                "x_wind_pl"
            ] * np.sin(ds["alpha"])

            # Calculating wind direction
            ds["wind_direction_pl"] = (
                np.rad2deg(
                    np.arctan2(-ds["eastward_wind_pl"], -ds["northward_wind_pl"])
                )
                + 360.0
            ) % 360.0

            # Calculating wind speed
            ds["wind_speed_pl"] = np.sqrt(
                (ds["eastward_wind_pl"] ** 2.0) + (ds["northward_wind_pl"] ** 2.0)
            )
